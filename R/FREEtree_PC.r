...
FREEtree_PC <- function(data, fixed_split, var_select, power, minModuleSize,
  ...
  if (Fuzzy == TRUE) {
    ...
    if (length(imp_var) > 1) {
      ...
      message("There is only one module ", final_var, "\n")
    }
  ...
  if (Fuzzy == FALSE) {
    ...
    if (length(imp_var) == 0) {
      ...
      mytree = lmertree(Formula, data = data, alpha = alpha_screen,
                        maxdepth = maxdepth)
      final_var = get_split_names(mytree$tree, data)
      message("There is only grey module ", final_var, "\n")
    }
    if (length(imp_var) == 1) {
      # only one non-grey module
      final_var = imp_var[[1]]
      message("There is only one non-grey module", final_var, "\n")
    ...
    if (length(imp_var) > 1) {
      ...
      message("After screening from non-grey modules ", final_var, "\n")
