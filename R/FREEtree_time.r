...
FREEtree_time <- function(data, fixed_regress, fixed_split, var_select,
  ...
  if (Fuzzy == TRUE) {
    ...
    if (length(imp_var) > 1) {
      ...
      message("after screening within modules", final_var, "\n")
      ...
      message("final features", final_var)
    } else {
      # only grey module
      message("There is only one module, final features", final_var)
    }
  ...
  if (Fuzzy == FALSE) {
    ...
    if (length(imp_var) == 0) {
      ...
      mytree = lmertree(Formula, data = data, alpha = alpha_screen,
                        maxdepth = maxdepth)
      final_var = get_split_names(mytree$tree, data)
      message("There is only one module which is grey, final features",
          final_var)
    } else {
      ...
      if (length(imp_var) > 1) {
        ...
        message("After screening within non-grey modules", final_var,
            "\n")
      ...
      message("The chosen non-grey features are", final_var, "\n")
      ...
      message("The chosen grey features are", grey_var, "\n")
